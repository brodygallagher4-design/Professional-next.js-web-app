...
export function WalletPage({ setPage }: { setPage: (page: Page) => void }) {
  ...
  const rateLabel = converted > 0
    ? `≈ ${exchangeRates[currency].symbol}${converted.toLocaleString(undefined, { maximumFractionDigits: 0 })}`
  ...
  return <div className="min-h-screen text-white" style={{ background: MBG, fontFamily: FONT }}>
    ...
    {funding && <div className="fixed inset-0 z-[90] flex items-center justify-center bg-black/75 p-4 backdrop-blur-[3px] sm:p-5"><div className="max-h-[80vh] w-full max-w-[380px] overflow-y-auto rounded-[20px] border border-[#142037] bg-[#07101f] p-4 shadow-[0_28px_90px_rgba(0,0,0,.55)]"><div className="flex items-start justify-between"><div className="flex items-center gap-2.5"><span className="flex h-9 w-9 items-center justify-center rounded-lg bg-[#4a1814] text-[#ff5a37]"><WalletMoneyIcon size={18}/></span><div><h2 className="text-[16px] font-bold tracking-[-.03em]">Add Funds</h2><p className="mt-0.5 text-[12px] text-[#a5b3c6]">Fund your wallet securely</p></div></div><button onClick={() => setFunding(false)} className="rounded-full p-1 text-white transition hover:bg-white/10"><Cancel01Icon size={21}/></button></div><div className="mt-4 grid grid-cols-2 rounded-[18px] bg-[#142034] p-1.5"><button onClick={() => setFundTab("local")} className={`flex h-9 items-center justify-center gap-2 rounded-[12px] text-[13px] font-bold transition ${fundTab === "local" ? "bg-black text-white" : "text-[#9ca9bc]"}`}><BankIcon size={18}/>Local</button><button onClick={() => setFundTab("crypto")} className={`flex h-9 items-center justify-center gap-2 rounded-[12px] text-[13px] font-bold transition ${fundTab === "crypto" ? "bg-black text-white" : "text-[#9ca9bc]"}`}><span className="flex h-5 w-5 items-center justify-center rounded-full bg-white text-[13px] font-black text-black">₿</span>Crypto</button></div>{fundTab === "local" ? <><div className="mt-3.5 flex items-center justify-between"><h3 className="text-[14px] font-bold">Select currency</h3><span className="text-[12px] text-[#acb9c9]">One currency per deposit</span></div><div className="mt-3 grid grid-cols-3 gap-2">{[["🇳🇬", "NGN", "Nigerian Naira"],["🇬🇭", "GHS", "Ghana Cedis"],["🇰🇪", "KES", "Kenya Shilling"],["🇿🇦", "ZAR", "South African Rand"],["🇨🇮", "XOF", "Ivory Coast"]].map(([flag, code, note]) => <button key={code} onClick={() => setCurrency(code)} className={`flex h-[42px] items-center gap-1.5 rounded-xl border px-2 text-left transition ${currency === code ? "border-[#f04e23] bg-[#391813]" : "border-[#1e2b3f] bg-black"}`}><span className="text-[18px]">{flag}</span><span><b className="block text-[12.5px]">{code}</b><small className="block text-[8px] text-[#9eabc0]">{note}</small></span></button>)}</div><label className="mt-3 block text-[13px] font-bold">Amount<div className="mt-1.5 flex flex-col items-center rounded-[14px] bg-[#08111e] py-1.5"><div className="flex items-center gap-2"><span className="text-[17px] text-[#9ca9bc]">$</span><input value={amount} onChange={(event) => setAmount(event.target.value)} inputMode="decimal" placeholder="0" onFocus={(event) => event.currentTarget.select()} className="w-28 bg-transparent text-center text-[22px] font-bold tracking-[-.04em] outline-none placeholder:text-[#71819a]"/></div><span className="text-[12px] font-medium text-[#c9d2df]">{rateLabel}</span></div></label><div className="mt-3 border-t border-[#263448]"/><div className="mt-2.5 flex gap-2.5 rounded-xl border border-[#f04e23] bg-[#4a1a17] px-3 py-2"><span className="shrink-0"><ProtectedShield size={20}/></span><div><h4 className="text-[12px] font-bold">Secured and Trusted</h4><p className="mt-0.5 text-[10px] leading-4 text-white">Your funds are protected with a bank level security and processed through a licensed payment partner</p></div></div><button onClick={beginDeposit} disabled={depositing} className="mt-3 h-10 w-full rounded-full bg-[#ff542f] text-[13px] font-bold transition hover:bg-[#ff6a4a] active:scale-[.98] disabled:opacity-60 inline-flex items-center justify-center gap-2">{depositing && <svg className="animate-spin" width="15" height="15" viewBox="0 0 24 24" fill="none"><circle cx="12" cy="12" r="9" stroke="#fff" strokeOpacity=".3" strokeWidth="2.6"/><path d="M21 12a9 9 0 0 0-9-9" stroke="#fff" strokeWidth="2.6" strokeLinecap="round"/></svg>}{depositing ? "Starting payment…" : "Continue to payment"}</button><p className="mt-2 text-center text-[9.5px] text-white/80">♙ You will be redirected to a secure service provider</p></> : <><h3 className="mt-5 text-[15px] font-bold">My static wallets</h3><div className="mt-3 grid grid-cols-3 gap-2 sm:grid-cols-4">{[["USDT", "#e62929"],["USDC", "#2775ca"],["BTC", "#f7931a"],["ETH", "#627eea"],["LTC", "#345dbe"],["BNB", "#f3ba2f"],["TRX", "#ef0027"]].map(([symbol, color]) => <CoinChip key={symbol} symbol={symbol} color={color} active={coin === symbol} onClick={() => setCoin(symbol)}/>)}</div><div className="mt-5 rounded-2xl border border-[#243149] bg-black p-4"><label className="block text-[13px] font-bold">Select Network<select className="mt-2 h-10 w-full rounded-md border border-[#34435b] bg-[#0c1627] px-3 text-[13px] outline-none"><option>TRC 20</option><option>ERC 20</option></select></label><div className="my-5 flex justify-center"><QRCode /></div><div className="flex min-w-0 items-center gap-2 rounded-xl bg-[#142034] px-3 py-3"><span className="min-w-0 flex-1 truncate text-[12px]">TBC1KZwjJ6dkhfZdgagnxfiAq2aPaevyUM</span><button onClick={() => copyText("TBC1KZwjJ6dkhfZdgagnxfiAq2aPaevyUM")} className="flex h-9 w-9 shrink-0 items-center justify-center rounded-full bg-[#ff5a37]"><Copy01Icon size={17}/></button></div><div className="mt-4 rounded-xl border border-[#ff4e2e] bg-[#451a18] px-4 py-4"><div className="flex gap-3"><Alert01Icon size={22} className="shrink-0 text-[#ff5a37]"/><div><h4 className="text-[12px] font-bold">Before you deposit</h4><ul className="mt-2 list-disc space-y-1 pl-4 text-[11px] leading-4"><li>Deposits below ~8 USD may not be processed.</li><li>Send only {coin} to this address.</li><li>Make sure you use the correct network. Incorrect network may lead to loss of funds.</li></ul></div></div></div></div></>}</div></div>}

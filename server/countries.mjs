// Country calling codes + national phone-number length rules (server copy —
// generated alongside src/app/lib/countries.ts; keep in sync).
...
export const countryByIso = (iso) => COUNTRIES.find((c) => c.iso === String(iso ?? "").toUpperCase());

export function validatePhone(iso, raw) {
  ...
  if (national.length < c.min || national.length > c.max) return { ok: false, error: "Enter a valid " + c.name + " phone number." };
